...
```{r chunk-label1, echo = TRUE, fig.cap = 'Do processing.'}
# Run process!
...
You will get something like this [output](./out/Barents Sea capelin acoustic abundance estimate in autumn/output/index.html).
...
outChart <- REDUStools:::genPNGplot(results, sts, stsOut, appendTimeStamp)
outTable <- REDUStools:::genTables(results, sts, stsOut, appendTimeStamp)
if (length(outTable) == 0) 
    return(NULL)
TS <- ""
if (!is.na(appendTimeStamp)) 
    TS <- paste0(".", appendTimeStamp)
